...
static void mode_ramp_setup_spin_coater(void)
{
	RampParams_t params = {0};
	params.minRpm = 1000.0f;
	params.timeMin = 10000;
	params.maxRpm = 3000.0f;
	params.timeMax = 30000;
	Mode_RampInit(params);
}
...
void loop()
{
	...
	float error = gCoaterControl.targetRpm - nowRPM;
	
	static uint32_t sDebugTime = 0;
	if (millis() - sDebugTime >= COATER_ENCODER_AVG_SAMPLE)
	{
		printf("SP=%.2f; RPM=%.2f; Err=%.2f; Duty: %d\n", (double)(gCoaterControl.targetRpm), (double)nowRPM, (double)error, nowDuty);
		sDebugTime = millis();
	}
}
